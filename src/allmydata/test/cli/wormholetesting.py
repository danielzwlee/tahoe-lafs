...
def _verify()-> None:
    ...
    testa = getfullargspec(create)
    testb = getfullargspec(MemoryWormholeServer.create)

    # I know it has a `self` argument at the beginning.  That's okay.
    testb = testb._replace(args=testb.args[1:])

    # getfullargspec returns more information than getargspec.
    # Just compare the same information to check function signature
    assert testa.varkw == testb.varkw
    assert testa.args == testb.args
    assert testa.varargs == testb.varargs
    assert testa.defaults == testb.defaults
